...
*/
	...
	static struct wake_lock aerial_wake_lock;
	struct work_struct aerial_work; 
	struct workqueue_struct *aerial_wq = NULL;
	u8 sar_state=UEVENT_OFF;
	/* DTS2015082408564 chendong wx241705 end > */
static struct adux1050_platform_data local_plat_data = {
	.init_regs = {
		0x0001010F,	0x00020529,	0x000301ff,	0x00050F55,
...
/* DTS2015030301495  dingjingfeng 20150303 end > */
	if (val == ADUX1050_ENABLE) {
		...
		if (ret) {
			dev_err(adux1050->dev, "%s: could not enable vdd voltage\n", __func__);
			return ret;
		}*/
		...
		if (ret) {
			dev_err(adux1050->dev, "%s: could not disable vdd voltage\n", __func__);
			return ret;
		}*/
...
{
	/* DTS2015072707140 chendong 20150810 begin> */
	...
	/* DTS2015072707140 chendong 20150810 end> */
	dev_info(adux1050->dev, "%s - prv_flag %d curr %d\n", __func__, adux1050->dev_enable, val);
	if (adux1050->dev_enable == val) {
		dev_info(adux1050->dev, "%s - Device already in %s state\n", __func__, dev_state[val]);
		goto exit;
	}
	if ((val == ADUX1050_ENABLE)&&!adux1050->aerial_status) {
	/* DTS2015082408564 chendong wx241705 end > */
		adux1050->dev_enable = val;
#ifdef CONFIG_ADUX1050_POLL
	 ...
	 struct device_attribute *attr, char *buf)
{
	...
	adux1050->read(adux1050->dev, INT_STATUS_REG, &status, DEF_WR);
	if (status & 0x1)
		status = 1;
	else if (status & 0x10)
		status = 2;
...
static ssize_t adux1050_saronline_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", sar_state);
}
...
static struct device_attribute dev_attr_sensor_dump =
	__ATTR(adux1050_status, S_IRUGO,
	       show_dumpregs, NULL);
static struct device_attribute dev_attr_sensor_saronline =
	__ATTR(saronline, S_IRUGO,
	       adux1050_saronline_show, NULL);
/* < DTS2015082408564 chendong wx241705 begin */
static struct device_attribute dev_attr_sensor_status =
	__ATTR(adux1050_prox_status, S_IWUGO | S_IRUGO,
...
static struct attribute *adux1050_attrs[] = {
	...
	&dev_attr_sensor_saronline.attr,
...
unsigned int get_aerial_status(unsigned int aerial_detection_gpio)
{
    int electrical_aerial;
	
    electrical_aerial = gpio_get_value(aerial_detection_gpio);
    pr_info(" electrical_lvl_sar = %d  \n", electrical_aerial);
    return electrical_aerial;
}
static void aerial_detect_work(struct work_struct *work)
{
	struct adux1050_chip *adux1050 =  container_of(work,
						   struct adux1050_chip, work);
    unsigned int new_status;
    char *aerial_on_duty[2]    = { "AERIAL_STATE=ON DUTY", NULL };
    char *aerial_off_duty[2]    = { "AERIAL_STATE=OFF DUTY", NULL };
    
    new_status = get_aerial_status(adux1050->gpio_pin);
    pr_info(KERN_INFO "aerial status change from %s to %s\n",
           adux1050->aerial_status == 1 ? "on duty":"off duty",
           new_status== 1 ? "on duty":"off duty" );

    if( new_status != adux1050->aerial_status )
    {
        adux1050->aerial_status = new_status; 
		/* DTS2015072707140 chendong 20150810 begin> */
		__set_enable(adux1050,!adux1050->aerial_status);
		schedule_work(&adux1050->calib_work);
		/* DTS2015072707140 chendong 20150810 end> */
        kobject_uevent_env(&adux1050->dev->kobj, KOBJ_CHANGE,( adux1050->aerial_status ? aerial_on_duty : aerial_off_duty));  
        pr_info("send aerial state change uevent........................end\n");
    }
    
    wake_unlock(&aerial_wake_lock);
    return;
}

static void timer_detect_func(unsigned long arg)
{
    pr_info("aerial -%s[%d]: %s\n", __FILE__, __LINE__, __FUNCTION__);  
    queue_work(aerial_wq, &aerial_work);
}
static irqreturn_t aerial_gpio_irq_handler(int irq, void *dev_id)
{
    struct adux1050_chip *adux1050 = (struct adux1050_chip *)dev_id;

    if (irq != adux1050->detect_irq)
        return IRQ_NONE;
    
    wake_lock(&aerial_wake_lock);
	/* DTS2015072707140 chendong 20150810 begin> */
    mod_timer(&adux1050->timer_detect, jiffies+HZ/2);
	/* DTS2015072707140 chendong 20150810 end> */
    return IRQ_HANDLED;
}
			...
			if (high_status_change & 1) {
				if (adux1050->high_status & 1) {
					...
					if (adux1050->send_event)
					  ...
					  /* < DTS2015082408564 chendong wx241705 begin */
						sar_state=UEVENT_ON;
						kobject_uevent_env(&adux1050->dev->kobj, KOBJ_CHANGE, sar_on_duty);
						pr_info("send sar state change uevent(ON DUTY)........................end\n");
				} else {
					dev_info(adux1050->dev, "St = %dx\n",
						 stg_cnt);
						sar_state=UEVENT_OFF;
						kobject_uevent_env(&adux1050->dev->kobj, KOBJ_CHANGE, sar_off_duty);
						pr_info("send sar state change uevent(OFF DUTY)........................end\n");
					if (adux1050->send_event)
				...
				if (adux1050->low_status & 1) {
					...
					if (adux1050->send_event)
						indicate_active_state(adux1050,
								      stg_cnt,
								      TH_LOW);
						/*kobject_uevent_env(&adux1050->dev->kobj, KOBJ_CHANGE, sar_low_threshold);
						pr_info("send sar state change uevent(cross low threshold)........................end\n");*/
				} else {
					dev_info(adux1050->dev, "St = %dx\n",
						 stg_cnt);
					if (adux1050->send_event)
		...
		const struct i2c_device_id *id)
{
	...
	if (ret) {
		pr_err("%s: could not register class device\n", __func__);
		goto err_input_register_device;
	}
	...
	adux1050->proxy_cancel = TRUE;
	/* DTS2015072707140 chendong 20150810 begin> */
	adux1050->gpio_pin = of_get_named_gpio_flags(adux1050->dt_device_node, "adi,aerial-detect", 0, NULL);
	dev_err(adux1050->dev, "defined aerial-detect = %d\n", adux1050->gpio_pin);
	if (!gpio_is_valid(adux1050->gpio_pin))
	{
		dev_err(adux1050->dev, "%s: Failed to parser dt gpio_pin\n", __func__);
		return 0;
	}
	adux1050->detect_irq = gpio_to_irq(adux1050->gpio_pin);
     if (adux1050->detect_irq < 0)
    {
        pr_err("%s: probe gpio_to_irq  error!\n", __func__);
        goto  err_free_irq;
    }
	ret = request_threaded_irq(adux1050->detect_irq, NULL,
				aerial_gpio_irq_handler,
				IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
				"aerial", adux1050);
				
	if (ret) {
		pr_err("%s: irq %d Driver init Failed", __func__, adux1050->detect_irq);
		//goto err_free_irq;
		}
	disable_irq(adux1050->detect_irq);
	init_timer(&adux1050->timer_detect);
    adux1050->timer_detect.function = &timer_detect_func;
	
	adux1050->aerial_status = get_aerial_status(adux1050->gpio_pin);
	/* < DTS2015082408564 chendong wx241705 begin */
	pr_err("aerial_status %d\n",adux1050->aerial_status );
	/* DTS2015082408564 chendong wx241705 end > */
	aerial_wq = create_workqueue("aerial_workqueue");
	if(!aerial_wq)
        goto err_free_irq;
	    
	INIT_WORK(&aerial_work, aerial_detect_work);
    wake_lock_init(&aerial_wake_lock, WAKE_LOCK_SUSPEND, "aerial detect"); 
...
err_kzalloc_mem:
	wake_lock_destroy(&aerial_wake_lock);
	kfree(adux1050);
	return ret;
}
...
*/
static int __init adux1050_module_init(void)
{
	pr_info("%s,START\n", __func__);
...
*/
static void __exit adux1050_module_exit(void)
{

...
#define CY_FW_MANUAL_UPGRADE_FILE_NAME "cyttsp5_fw_manual_upgrade.bin"
		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	if (fw_ver_new > fw_ver_img) {
		tp_log_info( "%s: Image is newer\n", __func__);
		return 1;
	}

	if (fw_ver_new < fw_ver_img) {
		tp_log_info( "%s: Image is older\n", __func__);
		return -1;
	}
...
static int cyttsp5_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
	...
	tp_log_info( "%s %d: enter to load app\n", __func__, __LINE__);
		...
		int fw_size)
{
	...
	tp_log_info( "%s %d: cyttsp5 upgrade firmware\n", __func__, __LINE__);
...
static int upgrade_firmware_from_class(struct device *dev)
{
...
#ifdef CONFIG_HUAWEI_KERNEL
	retval = request_firmware(&fw_entry, CY_FW_MANUAL_UPGRADE_FILE_NAME, dev);
	if (retval) {
	...
	_cyttsp5_firmware_cont(fw_entry, dev);
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	if (!retVal) {
		tp_log_info("%s %d:firmware have upgraded\n", __func__, __LINE__);
	} else {
#if defined (CONFIG_HUAWEI_DSM)
		if (!dsm_client_ocuppy(tp_cyp_dclient)) {
			...
			dsm_client_notify(tp_cyp_dclient, DSM_TP_FW_ERROR_NO);
		} else {
			tp_log_err("%s %d: tp dsm client is busy!!\n", __func__, __LINE__);
		}

#endif
		tp_log_info("%s %d:firmware haven't upgraded, rc = %d\n", __func__, __LINE__, retVal);
	}
#endif
	...
	/* when firmware in ic missing, ld->si will be NULL, this issue will lead to ttconfig upgrade fail,
		to fix this issue, after upgrade bin, we should erquest system info agin */
	ld->si = cmd->request_sysinfo(dev);
	...
	if (!retVal) {
		tp_log_info("%s %d:ttconfig have upgreded\n", __func__, __LINE__);
	} else {
		tp_log_info("%s %d:ttconfig haven't upgrade, rc = %d\n", __func__, __LINE__, retVal);
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	tp_log_err("%s %d:Enter manual_upgrade\n", __func__, __LINE__);

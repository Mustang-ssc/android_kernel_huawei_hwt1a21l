		...
		struct device_node *core_node)
{
	...
	if (!rc) {
		...
		tp_log_debug("%s %d:pdata->level_irq_udelay = %d.\n", 
					__func__, __LINE__, pdata->level_irq_udelay);
	} else {
		tp_log_err("%s %d:[cy,level_irq_udelay] read fail, rc = %d.\n", 
					__func__, __LINE__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_debug("%s %d:pdata->vendor_id = %d.\n", 
					__func__, __LINE__, pdata->vendor_id);
	} else {
		tp_log_err("%s %d: [cy,vendor_id] read fail, rc = %d.\n", 
					__func__, __LINE__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_debug("%s %d:pdata->product_id = %d.\n", 
					__func__, __LINE__, pdata->product_id);
	} else {
		tp_log_err("%s %d: [cy,product_id] read fail, rc = %d.\n", 
					__func__, __LINE__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_debug("%s %d:pdata->easy_wakeup_gesture = %d.\n", 
					__func__, __LINE__, pdata->easy_wakeup_gesture);
	} else {
		tp_log_err("%s %d: [cy,easy_wakeup_gesture] read fail, rc = %d.\n", 
					__func__, __LINE__, rc);
	}

	/* < DTS2014122209378 caowei 201401222 begin */
	rc = of_property_read_u32(core_node, "cy,easy_wakeup_supported_gestures", 
			&value);
	if (!rc) {
		pdata->easy_wakeup_supported_gestures = value;
		tp_log_debug("%s:easy_wakeup_supported_gestures = %d.\n", __func__, value);
	} else {
		tp_log_err("%s:[easy wakeup supported gestures] read fail, rc = %d.\n", __func__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_err("%s:[cy,double_tap_enabled] read fail, rc = %d.\n", __func__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_err("%s:[cy,dtz_x0] read fail, rc = %d.\n", __func__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_err("%s:[cy,dtz_y0] read fail, rc = %d.\n", __func__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_err("%s:[cy,dtz_x1] read fail, rc = %d.\n", __func__, rc);
	}
	...
	if (!rc) {
		...
		tp_log_err("%s:[cy,dtz_y1] read fail, rc = %d.\n", __func__, rc);
	}
	...
	pdata->glove_suppert = 0;
	rc = of_property_read_u32(core_node, "cy,glove_support", &value);
	if (!rc) {
		pdata->glove_suppert = value;
	}
	...
	tp_log_debug("%s: irq_gpio:%d rst_gpio:%d\n"

...
#define  FACTORY_MODE "androidboot.huawei_swtype=factory"
#define  NORMAL_MODE "androidboot.huawei_swtype=normal"
...
static inline void _get_sup_clk_tbl(struct mmc_host *mmc, unsigned int **tbl, u8 *cnt)
{
...
static inline void _set_sup_clk_tbl(struct mmc_host *mmc, unsigned int *tbl, u8 cnt)
{
	struct sdhci_host *host = mmc_priv(mmc);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
...
_modify_support_clock_table(struct mmc_host *host)
{
	...
	cnt = sizeof(g_modify_sup_clk_tbl) / sizeof(u32);
...
/*get device's work mode
 * when in factory, return 0; when in normal is 1, other is -1
 */
static int get_device_run_mode(void){
	char cmdline[512] = {0};
	mm_segment_t old_fs;
	struct file *fp = NULL;
	loff_t pos = 0;
	char cmd_line[] = "/proc/cmdline";
	int read_count = 0;

	fp = filp_open(cmd_line, O_RDONLY, 0);
	if(IS_ERR(fp)){
		pr_err("[HW]:%s:%s open failed.", __func__, cmd_line);
		return -1;
	}
	...
	read_count = vfs_read(fp, cmdline, sizeof(cmdline), &pos);
	if(read_count <= 0){
	...
	if(NULL == strstr(cmdline, FACTORY_MODE))
	{
		if(NULL != strstr(cmdline, NORMAL_MODE)){
			pr_err("device is working in normal mode\n");
			return 1;
		}else{
			pr_err("device is working in normal mode\n");
			return -1;
		...
		pr_info("device is working in %s mode\n", FACTORY_MODE);
		return 0;
	}
}
...
static int __init mmc_hw_aging_test_init(void)
{
	...
	int ret;
	/*check device is working in factory mode, or insmod fail*/
	if(0 != get_device_run_mode())
	{
		pr_err("this module only work in factory mode, so insmode failed");
		goto err;
	}else{
		pr_info("current device is in factory mode, so insmode this module");
	}
	...
	//parse mmc card addr.
	ret = kstrtoint(buf, 0, &mmc_card_addr);
	if(ret){
		pr_err("[HW]:%s:parse card_adrr error buf=%s.", __func__, buf);
		goto err;
	}

	mmc_card_addr ^= CARD_ADDR_MAGIC;
